Create Codex pet row `running-right` for `chii`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Chiikawa-inspired but original: tiny round white kawaii pet, soft plush/sticker feel, warm gray rounded ears, simple dot eyes, tiny mouth, rosy cheeks, small paws, gentle anxious-curious charm, no copied character markings or exact Chiikawa design.. Preserve silhouette, face, palette, material, proportions, markings, and props.
